...
A JSON Web Token is sent back from the server in exchange for correct login credentials. In this way, the app can protect its data from unauthorized access.

- [ ] What steps can you take in your web apps to keep your data secure?

Require login credentials and proper authentication before the API returns data.

- [ ] Describe how web servers work.

Web servers receive http requests and send back responses depending on how those requests are formatted. For example, when a user first logs in, the server will respond with an auth token which will then be sent back to the client side app that then stores it in local storage for use in future requests. In any subsequent requests, the client side app will grab the token from local storage and add it as an auth header to the request so that the server knows the user is authorized to make the request and receive the data for which they're asking.
